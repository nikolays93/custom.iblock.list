...
class CatalogSectionComponent extends ElementList
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		// for what?
		$exclude = array(
			'USE_PRICE_COUNT', 'SHOW_PRICE_COUNT', 'PRICE_VAT_INCLUDE',
			'BASKET_URL', 'ADD_PROPERTIES_TO_BASKET', 'ADD_TO_BASKET_ACTION',
			'CONVERT_CURRENCY', 'DISPLAY_COMPARE', 'HIDE_NOT_AVAILABLE_OFFERS',
			'MESS_BTN_ADD_TO_BASKET', 'MESS_BTN_BUY', 'MESS_NOT_AVAILABLE',
			'OFFERS_LIMIT', 'PRICE_CODE', 'OFFERS_FIELD_CODE',
			'OFFERS_PROPERTY_CODE', 'OFFERS_SORT_FIELD', 'OFFERS_SORT_ORDER',
			'OFFERS_SORT_FIELD2', 'OFFERS_SORT_ORDER2', 'COMPARE_PATH',
			'COMPARE_NAME', 'USE_COMPARE_LIST', 'CURRENCY_ID', 'ACTION_VARIABLE',
			'PRODUCT_ID_VARIABLE', 'PRODUCT_QUANTITY_VARIABLE',
			'PRODUCT_PROPS_VARIABLE',
		);

		foreach ($exclude as $exclude_code) {
			unset($params[ $exclude_code ]);
		}
